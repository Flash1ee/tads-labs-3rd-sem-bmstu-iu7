 ...
 * @return int 
 */
int clean_mantis(mantissa_t *number, size_t DIGITS_LEN)
{
    if (!number)
 ...
 * @return int 
 */
int parse_mantis(char *string, mantissa_t *number, char **mantis_end, int *dot_pos)
{
    int dots_count = 0;
    int dot_position = 0;
    int input_len = 0;
    ...
    if (*string == '-' || *string == '+')
    {
        ...
        if ((*string == '-' || *string == '+') && strlen(string) < 3 && !isdigit(string[1]) )
            return INPUT_ERROR;
        string++;
    }
    if ((!isdigit(*string) && (strlen(string) > 1 && !isdigit(string[1])))|| *string == 'e' || *string == 'E')
        return INPUT_ERROR;
...
int fprint_long_real(long_real_t *number, FILE *file)
{
    ...
    size_t zeros = 0;
    size_t j = MANTIS_LENGTH;
    while (j > 0)
    {
        if (number->mantis.digits[j] == 0)
            zeros++;
        if (number->mantis.digits[j] != 0 && zeros)
            break;
        j--;
    }

        
    for (; i < MANTIS_LENGTH - zeros; i++)
...
/**
 * @brief Преобразование целого длинного к длинному вещественному
 * 
 * @param lreal 
 * @param lint 
 * @return int 
 */
int mantissa_to_long_real(long_real_t *lreal,const mantissa_t *lint)
{
    if (!lreal || !lint)
        return POINTER_ERROR;
 ...
 * @return int 
 */
int mantis_shift_left_side(mantissa_t *number, int *shift)
{
    *shift = 0;
 ...
 * @return int 
 */
int mantis_shift(mantissa_t *number, int *shift)
{
    if (!number || !shift)
        return POINTER_ERROR;
    if (!(*shift))
 ...
 * @return int 
 */
int add_long_int(mantissa_t *first, const mantissa_t *second)
{
 ...
 * @return int 
 */
int sub_long_int(mantissa_t *first, const mantissa_t *second)
{
    for (int i = MANTIS_LENGTH - 1; i >= 0; i--)
    {
 ...
 */
int cmp_long_int(const mantissa_t *first, const mantissa_t *second)
{
    for (size_t i = 0; i < MANTIS_LENGTH; i++)
    {
...
int div_long_real(const long_real_t *first, const long_real_t *second, long_real_t *result)
{
    ...
    mantissa_t work_num = first->mantis;
